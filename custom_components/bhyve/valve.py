...
async def async_setup_entry(
    ...
    async def async_service_handler(service):
        ...
        if entity_ids:
            target_entities = [component.get_entity(entity) for entity in entity_ids]
        else:
            return
...
class BHyveZoneValve(BHyveDeviceEntity, ValveEntity):
    ...
    async def start_watering(self, minutes):
        """Turns on the valve and starts watering."""
    ...
    async def stop_watering(self):
        """Turns off the valve and stops watering."""
        station_payload = []
        self._is_on = False
        await self._send_station_message(station_payload)
    ...
    async def async_turn_on(self, **kwargs: Any) -> None:
        ...
        if run_time == 0:
            _LOGGER.warning(
                "Valve %s manual preset runtime is 0, watering has defaulted to %s minutes. Set the manual run time on your device or please specify number of minutes using the bhyve.start_watering service",
                self._device_name,
